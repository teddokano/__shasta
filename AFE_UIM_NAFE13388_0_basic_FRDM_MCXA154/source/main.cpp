...
//	dev.write_r16( AO_CAL_COEF, 0x1000 );
	
	dev.write_r16( AIO_PROT_CFG, 0x7 << 13 | 0x0 << 11 | 0x3 << 9 | 0x1 << 8 | 0x3 << 6 );
	dev.write_r16( AO_SLR_CTRL, 0x2A00 );
...
//	dev.write_r16( AO_SYSCFG, 0x0C00 );
	...
	dev.write_r16( 0x30, 0x0000 );
#endif
		dev.write_r24( AO_OC_POS_LIMIT, ((1 << 17) - (1 << 10)) << 6 );
		dev.write_r24( AO_OC_NEG_LIMIT, ~(((1 << 17) - (1 << 10)) << 6) );
//	dev.write_r24( AO_OC_NEG_LIMIT, -131071 << 5 );
//	dev.write_r24( AO_OC_NEG_LIMIT, 0x7FFFFF );
	
	dev.read_r16( PN2 );
//	dev.write_r24( AO_DATA, 0xF645C0 );
	dev.write_r24( AO_DATA, 0x7FFFC0 );
	...
	while ( true )
	{
		dev.write_r24( AO_DATA, count << 10 );
		printf( "%ld:  ", count++ );
...
;	
	...
	while ( true )
	{
		uint32_t	value	= 104858;
		
		dev.write_r24( AO_DATA, value );
		printf( "0x%04X\r\n", dev.read_r16( AIO_STATUS ) );
		dev.write_r16( CMD_AO_ABORT );
		printf( "0x%04X\r\n", dev.read_r16( AIO_STATUS ) );
		wait( 2.0 );
